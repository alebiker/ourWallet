...
public class Ocr {
    ...
    public Job enqueue(Bitmap bitmap) {
        ...
        Log.d(TAG, "Enqueuing "+bitmap.getHeight()+"x"+bitmap.getWidth() + " bitmap");
    ...
    private void connectOcrService(final InitCallback init) {
                ...
                try {
                    ...
                    if (!mStorageAvailable) {
                        ...
                        if (!mSuppressAlerts) {
                            ...
                            it.alborile.mobile.ocr.client.VersionAlert.createStorageAlert(mContext.get(), onClick).show();
                        } else {
                            postInitialized(init, STATUS_MISSING);
                        }
                    ...
                    if (languages == null || languages.isEmpty()) {
                        ...
                        if (!mSuppressAlerts) {
                            ...
                            it.alborile.mobile.ocr.client.VersionAlert.createLanguagesAlert(mContext.get(), onClick, onClick)
                                    .show();
                        } else {
                            postInitialized(init, STATUS_MISSING);
                        }
        ...
        Intent intent = new Intent(Intents.Service.ACTION_OCR);
    ...
    private final IOcrCallback mCallback = new IOcrCallback.Stub() {
        ...
        public void onResult(final long token, final OcrResult result) {
            if (mOnResult != null) {
                mOnResult.onResult(token, result);
            }
        }
    };
    ...
    public static class Parameters implements Parcelable {
        ...
        public Parameters(Parameters paramToClone) {
            ...
            setFlag(FLAG_SPELLCHECK,paramToClone.getFlag(FLAG_SPELLCHECK));
            setFlag(FLAG_DEBUG_MODE,paramToClone.getFlag(FLAG_DEBUG_MODE));
            setFlag(FLAG_DETECT_TEXT,paramToClone.getFlag(FLAG_DETECT_TEXT));
            ...
            setLanguage(paramToClone.getLanguage());

            setPageSegMode(paramToClone.getPageSegMode());
        }
        ...
        public void setPageSegMode(int pageSegMode) {
            if (pageSegMode < 0 || pageSegMode > PSM_COUNT) {
