...
/**
 * Find all characters and symbols in pix and returns as PIXA with relative box info
 */
l_int32 ConnCompValidPixa(PIX *pix8, PIX *pix, PIXA **ppixa, NUMA **pconfs,
  ...
  PIX *pixt1, *pixt2, *pixt3, *pixt4, *pixt5,*pixdb, *pixdb8;
  PIXA *pixa, *pixasort;
  NUMA *confs, *confsort;
  BOX *box;
  BOXA *boxa;
  ...
  *pconfs = NULL;
  ...
  if ((pixdb = pixCopy(NULL, pix)) == NULL)
      return ERROR_INT("pixt2 not made", procName, 1);

  //debug
	pixdb8 = pixConvert1To8(NULL,pixdb,0,1);
	pixSetColormap(pixdb8, cmap);
  ...
  while (1) {
    ...
    pixRenderBoxArb(pixdb8,box,1,0,0,255);
    ...
    //check if it is enought good component to be added or is just noise
    if (ValidateSingleton(pixt3, box, pixt5, &singleton_conf, params)) {
      ...
      //debug
      pixRenderBoxArb(pixdb8,box,1,0,255,0);
    } else {
      pixRenderBoxArb(pixdb8,box,1,255,0,0);
      boxDestroy(&box);
      pixDestroy(&pixt3);
    }
  ...
  //debug
  sprintf(filename, "%s/%ld_components.jpg", "./runtime_debug",(long int ) time(NULL));
  pixWriteImpliedFormat(filename, pixdb8, 85, 0);
...
/**
 * Remove the Components that doesn't have any valid partner(pair), nor left nor right.
 * Flag the invalid in the remove array
 */
...
l_int32 MergeClusterPartners(PIX *pix8, PIXA *pixa, NUMA *confs, l_uint8 *remove, l_int32 *left, l_int32 *right,
  ...
  PIX *pix, *pixd, *pix_cluster, *pixdb;
  BOX *box, *boxd;
  ...
  *pclusterconfs = confd;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (ValidateCluster(pix_cluster, pixa_cluster, boxd, &cluster_conf, params)) {
      ...
      //debug
      pixRenderBoxArb(pixdb,boxd,1,0,255,0);
